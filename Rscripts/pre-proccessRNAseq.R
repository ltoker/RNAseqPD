...
if(length(c(Fgene, Mgene)) == 0){
  ...
  if(length(unique(Metadata$Sex)) == 2 | length(unique(Metadata$Sex)) == 0){
    ...
    genes <- unique(c(Fgene, Mgene))
    if(length(genes) == 0){
      genes = "COX6A2"
      print("No relevant sex genes in the data, using COX6A2 to define noise threshold")
    } else {
      print("All samples are of the same sex, noise threshold is based on Meta sex")
    }
    
    Noise <- sapply(genes, function(gene){
      SampleName = names(aned_good[,sapply(aned_good[1,], function(x) is.numeric(x))])
      SampleName <- SampleName[!SampleName %in% RmSample]
      temp <- aned_good %>% filter(GeneSymbol == gene) %>%
        select_(.dots=SampleName)  %>% unlist
      #quantile(temp, 0.9)
    })  %>% unlist
    MaxNoise <- quantile(Noise, 0.95, na.rm = TRUE)
  }
...
CorSamplesGood <- cor(aned_good[-c(1:3)], method = "spearman")
...
CorSamplesHigh <- cor(aned_high[-c(1:3)], method = "spearman")
...
CorSamplesLow <- cor(aned_low[-c(1:3)], method = "spearman")
...
exclude_samples_All <- OutSamples(MedianCorGood)

#Criteria for sample exclusion: Outliers based on all genes. 
exclude_samples <- exclude_samples_All
...
if(ncol(aned_good) > 20){
  ...
  if(exclude_samples != "none" ){
    print(paste0(paste0(exclude_samples, collapse = ", "), "are suspected outliers, no outliers removed")
)  }
}
